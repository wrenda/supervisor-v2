...
//KADEMELER
member.roles.add("997653749709414490"); //1. Yetkili Kademesi

//YETKİLER
member.roles.add("997653756231569449"); //1. Yetki 
...
let Wrenda = new Discord.MessageEmbed() 
.setColor("RANDOM")
.setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
...
• Yetki Verilen: <@!${member.id}> \`${member.id}\`
• Yetki Verilme Tarihi: \`${yetkialinmatarihi}\``)
.setFooter("Wrenda Was Here!", client.user.avatarURL({dynamic: true}))
return message.channel.send(Wrenda).then(m => m.delete({timeout: 10000}));
