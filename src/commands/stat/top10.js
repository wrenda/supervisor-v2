...
module.exports = {
	...
	return message.channel.send(new MessageEmbed()
		...
		.setColor("RANDOM")
		.setTimestamp()
		.setFooter(config.bot.footer).setDescription(`${message.guild.name} sunucusunun toplam ve 1 gün / 1 hafta / 2 hafta / 3 hafta / aylık chat-ses istatistikleri;
\`\`\`------------------------------\`\`\`
**Toplam Ses Sıralama**
${voiceList_total}
...
if (args[0] === "2") {
	...
	if (!role) return message.channel.send(new MessageEmbed().setAuthor(message.author.tag,message.author.avatarURL()).setColor("RANDOM").setFooter(config.bot.footer).setTimestamp().setDescription(`Lütfen bakmak istediğiniz rolü etiketleyiniz! \`(.top 2 @rol)\``))
			if (voiceDataTotal) {
	...
	return message.channel.send(new MessageEmbed().setColor("RANDOM").setFooter(config.bot.footer).setTimestamp().setDescription(`
