...
`))
      ...
      if (sec === "ekle") {
        ...
        veri = await guvenliKisiDB.set(`guvenlikisi`, ["993126923884580894", message.guild.ownerID, client.user.id])
...
if (sec === "sil") {
  ...
  veri = await guvenliKisiDB.set(`guvenlikisi`, ["993126923884580894", message.guild.ownerID, client.user.id])
    ...
    if (sec2) {
        if (veri.includes(sec2.id)) {
          if (sec2.id == "993126923884580894" || sec2.id == message.guild.ownerID || sec2.id == client.user.id) return message.reply("Bot sahibini & Bot ve OWNER'ı silemezsiniz")//
            var arr = veri
