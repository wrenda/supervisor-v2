...
readdir("./src/commands/", (err, files) => {
    ...
    files.forEach(f => {
        readdir("./src/commands/" + f, (err2, files2) => {
            ...
            files2.forEach(file => {// Wasley / Hesperos
                let prop = require(`./src/commands/${f}/` + file);
                console.log(`[Wrenda-Commands] ${prop.name} yüklendi!`);
...
readdir("./src/events", (err, files) => {
    ...
    files.filter((file) => file.endsWith(".js")).forEach((file) => {
        ...
        console.log(`[Wrenda-Events] ${prop.conf.name} yüklendi!`);
    });
});
...
client.on("message", async message => {
    if (message.content === "!buton-rol" && message.author.id === config.bot.owner) {
        ...
        const Activity = new buttons.MessageButton()
            .setStyle("green")// Wasley / Hesperos
...
client.on('message', async message => {// Wasley / Hesperos
if (message.content === 'wrenda!fakekatıl') { 
...
client.on("messageDelete", async (message) => {
    ...
    let snipe = {
        ...
        starihi: Date.now(),
        kanal: message.channel.id// Wasley / Hesperos
    }
...
client.on("message", async message => {
  ...
  if (data.includes(message.content)) {
    message.reply("Sanada Günaydın Canım. 😯🌄🌅");// Wasley / Hesperos
  }
});
...
client.on("message", async message => {
  ...
  if (data.includes(message.content)) {// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
...
client.on("voiceStateUpdate", async (oldState, newState) => {
  ...
  if((oldState.member && oldState.member.user.bot) || (newState.member && newState.member.user.bot)) return; // Wasley
  ...
  if(oldState.channelID && !newState.channelID) { // Kanaldan Ayrılınca
      ...
      if (oldState.channelID === await sunucuayarDB.get(`sunucuayar.sleep_kanal`)) return await sesdb.add(`sleep_room.${oldState.guild.id}.${oldState.id}.${await sunucuayarDB.get(`sunucuayar.sleep_kanal`)}`, duration); // Wasley
      // GENEL
      await sesdb.add(`stats.${oldState.guild.id}.${oldState.id}.channels.${oldState.channelID}`, duration);
      await sesdb.add(`stats.${oldState.guild.id}.${oldState.id}.category.${oldState.channel.parentID}`, duration); // Wasley / Hesperos
...
client.on("message", async (msg) => {
  ...
  let mstats_daily = new qdb.table("mstats_daily")// Wasley / Hesperos
  ...
  if (msg.author.bot) return// Wasley / Hesperos
...
client.on('clickButton', async (button) => {
  ...
  const onybuton = new disbut.MessageButton()
    ...
    const data = await db.get(`basvur.${button.message.id}`)// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
    if(!data) return;
    ...
    .setColor('RANDOM');// Wasley / Hesperos  https://discord.gg/hBRHeTssX5




  if(button.id === 'onay'){
    button.reply.defer()// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
  ...
  if(button.id === 'red'){
    ...
    button.channel.send(sorular[sayac++])// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
    ...
    .setAuthor('Wrenda was here?', button.message.guild.iconURL({dynamic: true, type: 'gif', size: 1024}))
    ...
    .setColor('RED');// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
    ...
    try {
      ...
      if (kelime.some(reklam => msg.content.includes(reklam))) {
        ...
        if (msg.deletable) msg.delete({
          timeout: 200// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
        }).catch(err => {});
      } else {
  ...
  if (reklamKoruma) {
    try {
      ...
      if (kelime.some(reklam => newMsg.content.includes(reklam))) {
        ...
        if (newMsg.deletable) newMsg.delete({// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
          timeout: 200
        }).catch(err => {});
      } else {
...
client.on("message", async function (msg) {
  ...
  if (kufurKoruma) {// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
    try {
...
client.on("message", async message => {
 ...
 // Wasley / Hesperos  https://discord.gg/hBRHeTssX5
client.channels.cache.get(config.channels.botdmlog).send(new Discord.MessageEmbed().setAuthor('Bota Dm Den Mesaj Gönderildi!').setFooter('Dm Log!').setDescription(`Gönderen kişi:   ${message.author.tag}`).setTimestamp().setThumbnail(client.user.avatarURL).addField("Mesajı;",
message.content).setColor("RANDOM"))/// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
})
...
"Solundaki kişinin poposuna şaplak at.","Sağındaki kişiyi yanağından öp.","5 dakika boyunca öpüş.","En sevdiğinin ünlünün taklidini yap.","Dışarı çık ve “I’m a Little Teapot” şarkısını bütün hareketleriyle birlikte söyle.","Sağındaki nesneyle ilgili bir hikaye uydur."
  ...
  .setEmoji("✔️")
  .setStyle("url")
  .setURL("https://discord.gg/mestia")// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
...
"Doğruluk mu Cesaret mi sorularında hiç yalan söyledin mi?","Bir ev işinden kurtulabilsen hangisi olurdu?","En sevdiğin şarkı ne?","Daha önce hiç nargile içtin mi? İçtiysen anlat!","Eğer yapabilsen her gün yiyeceğin yemek ne olurdu?","En çirkin arkadaşın kim?","Ne tarzda lezzetli bir atıştırmalık sizi tahrik eder?","Hayatın boyunca kaç tane en iyi arkadaşın oldu?","Gittiğin en güzel yer neresi?","Banliyöde bir evde mi yaşamak istersin yoksa büyük bir şehirde apartman dairesinde mi?","Kız arkadaşının/Erkek arkadaşının en sevdiğin tarafı nedir?","30 yaşındaki seni anlat.","Daha önce birine verdiğin en kötü hediye nedir?","Sol tarafındaki kişiyi öpme düşüncesi seni heyecanlandırıyor mu?","Büyükannen ve büyükbabanın söylediği en komik şey nedir?","Bir günlüğüne bir işi denesen hangisini denerdin?","Hangi kurgusal karakterle evlenmek isterdin?","En kötü alışkanlıkların neler?","Hiç bir grup kurma / restoran açma konusunda hayal kurdun mu? Adı ne olurdu?","Daha önce hiç annenle babanı bastın mı?","Herhangi bir hayvan olabilsen hangisi olurdun?","Eğer kendi isminden başka bir isim alacak olsaydın neyi seçerdin?"]
    ...
    .setEmoji("✔️")
    .setStyle("url")
    .setURL("https://discord.gg/phedora")
    .setLabel("Destek Sunucusu")
    const embed = new Discord.MessageEmbed()
    .setTitle('Doğruluk Cesaret')
    .setDescription(`İşte Doğruluk Sorun : **${dogruluk[random]}**`)
    .setColor('GREEN')
    .setFooter(client.user.username ,client.user.avatarURL())// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
    .setTimestamp()
    const buttonRow = new disbut.MessageActionRow()
    .addComponent(buton)
    button.reply.send({embed : embed , component : buttonRow})
...
client.on('guildUpdate', async (oldGuild, newGuild) => {
    if (oldGuild.vanityURLCode === newGuild.vanityURLCode) return;// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
    ...
    request(settings, (err, res, body) => {
        if (err) {// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
...
const csri = config.roles.ticketdestek// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
client.on("clickButton", async button => {
...
.setLabel("Kapat")
.setEmoji("🔒")
.setID("Kilit");// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
//------------\\
...
await qdb.delete(`destekkanal_${kanal.id}`);// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (newState.channel != null && newState.channel.name.startsWith('➕│3 Kişilik Oda')) {newState.guild.channels.create(`🎧 ${newState.member.displayName}`, {type: 'voice',
    parent: newState.channel.parent,})
   .then((cloneChannel) => {newState.setChannel(cloneChannel);// Wasley / Hesperos  https://discord.gg/hBRHeTssX5
...
}, 60*1000*1)// 1 dakikada verileri siler yani 1 dakikada kullanıcı 5 sefer çık gir yaparsa sunucudan banlanır
